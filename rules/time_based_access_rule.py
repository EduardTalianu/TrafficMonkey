            ...
            for src_ip, connections in out_of_hours.items():
                if len(connections) >= self.alert_threshold:
                    ...
                    for i, conn in enumerate(connections[:5]):
                        ...
                        details.append(f"  {time_str}: {src_ip} -> {dst_ip}:{dst_port} ({bytes_transferred/1024:.1f} KB)")
                    
                    if details:
                        alerts.append("Recent connections:")
                        alerts.extend(details)
                        
                    if len(connections) > 5:
                        alerts.append(f"  ... and {len(connections) - 5} more")
            
            return alerts
        except Exception as e:
            return [f"Error in Time-Based Access rule: {str(e)}"]
