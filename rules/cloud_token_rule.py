...
class CloudTokenExtractorRule(Rule):
            ...
            for row in db_cursor.fetchall():
                ...
                if "eyJ" in header_value:
                    ...
                    if jwt_match:
                        ...
                        # Add as a red finding
                        severity = "medium"
                        if "exp" in token_info and "iat" in token_info:
                            # Check token expiration and lifetime
                            if token_info["exp"] - token_info["iat"] > 86400:  # > 24 hours
                                severity = "high"  # Long-lived tokens are higher risk
                        
                        details = {
                            "token_type": token_type,
                            "cloud_provider": token_info.get("cloud_provider", "Unknown"),
                            "token_snippet": self._mask_token(jwt_token),
                            "header_name": header_name,
                            "issuer": token_info.get("iss", "Unknown"),
                            "subject": token_info.get("sub", "Unknown"),
                            "expiration": token_info.get("exp", "Unknown"),
                            "issued_at": token_info.get("iat", "Unknown")
                        }
                        
                        remediation = "Review if this token grants excessive privileges. Ensure token lifetime is minimized and scoped appropriately. Rotate tokens if exposed to unauthorized systems."
                        
                        self.add_red_finding(
                            src_ip=src_ip,
                            dst_ip=dst_ip,
                            description=f"Cloud service JWT token ({token_type}) detected",
                            severity=severity,
                            details=details,
                ...
                for token_type, pattern in self.aws_patterns.items():
                    ...
                    for match in matches:
                        ...
                        # Add as a red finding
                        severity = "critical" if token_type == "secret_key" else "high"
                        details = {
                            "token_type": token_type,
                            "cloud_provider": "AWS",
                        ...
                        remediation = "Immediately rotate this AWS credential. Review IAM permissions and ensure principle of least privilege. Use temporary credentials or IAM roles where possible instead of long-lived access keys."
                        
                        self.add_red_finding(
                            src_ip=src_ip,
                            dst_ip=dst_ip,
                            description=f"AWS {token_type} exposed in HTTP traffic",
                            severity=severity,
                            details=details,
                            connection_key=connection_key,
                            remediation=remediation
                ...
                for token_type, pattern in self.azure_patterns.items():
                    ...
                    for match in matches:
                        ...
                        # Add as a red finding
                        severity = "critical" if token_type == "connection_string" else "high"
                        details = {
                            "token_type": token_type,
                        ...
                        remediation = "Rotate this Azure credential immediately. For connection strings, regenerate the account keys. For SAS tokens, create new tokens with appropriate time limits and permissions. Review Azure RBAC assignments."
                        
                        self.add_red_finding(
                            src_ip=src_ip,
                            dst_ip=dst_ip,
                            description=f"Azure {token_type} exposed in HTTP traffic",
                            severity=severity,
                            details=details,
                            connection_key=connection_key,
                            remediation=remediation
                ...
                for token_type, pattern in self.gcp_patterns.items():
                    ...
                    for match in matches:
                        ...
                        remediation = "Immediately rotate this GCP credential. For API keys, create a new key with appropriate restrictions and delete the exposed key. For service accounts, consider using workload identity federation instead of service account keys."
                        
                        self.add_red_finding(
                            src_ip=src_ip,
                            dst_ip=dst_ip,
                            description=f"GCP {token_type} exposed in HTTP traffic",
                            severity=severity,
                            details=details,
                            connection_key=connection_key,
                            remediation=remediation
    ...
    def _check_token_string(self, value, key_name, connection_key, src_ip, dst_ip, alerts):
        ...
        # Add as red finding
        severity = "high"
        if cloud_type == "AWS" and token_type == "secret_key":
            severity = "critical"
        elif cloud_type == "Azure" and token_type == "connection_string":
            severity = "critical"
        elif cloud_type == "GCP" and token_type == "api_key":
            severity = "critical"
        ...
        details = {
            ...
            "key_name": key_name,
            "masked_token": masked_token,
            "detection_location": "json_body"
        }
        
        # Add JWT-specific details if applicable
        if value.startswith('eyJ') and '.' in value:
            jwt_info = self._parse_jwt(value)
            for key in ["iss", "sub", "exp", "iat"]:
                if key in jwt_info:
                    details[key] = jwt_info[key]
        
        # Customize remediation based on token type
        if cloud_type == "AWS":
            remediation = "Immediately rotate this AWS credential. Review IAM permissions and implement least privilege. Consider using temporary credentials via STS or IAM roles instead of long-lived access keys."
        elif cloud_type == "Azure":
            remediation = "Rotate this Azure credential immediately. Update key vaults or configuration stores that may contain this credential. Review access controls and implement managed identities where possible."
        elif cloud_type == "GCP":
            remediation = "Rotate this Google Cloud credential immediately. Consider using workload identity federation or service account impersonation instead of service account keys. Ensure API keys have appropriate restrictions."
        else:
            remediation = "Rotate this cloud credential immediately. Review access controls and implement the principle of least privilege. Consider using managed identities or temporary credentials where possible."
        
        self.add_red_finding(
            src_ip=src_ip,
            dst_ip=dst_ip,
            description=f"{cloud_type} {token_type} token exposed in JSON data",
            severity=severity,
            details=details,
            connection_key=connection_key,
            remediation=remediation
