...
class ClearTextPasswordRule(Rule):
                        ...
                        for key, value in headers.items():
                            ...
                            for param in self.password_params:
                                if param in key_lower:
                                    ...
                                    # Add as a red finding with more details
                                    details = {
                                        "credential_type": "password",
                                        "field_name": key,
                                        "credential_value": self._mask_credential(value),
                                        "host": host,
                                        "uri": uri,
                                        "method": method,
                                        "detection_location": "json_header"
                                    }
                                    
                                    remediation = "Review this application's authentication mechanism. Implement HTTPS if not already in use. Consider using secure authentication tokens instead of sending credentials in plaintext."
                                    
                                    self.add_red_finding(
                                        src_ip=src_ip,
                                        dst_ip=dst_ip,
                                        description="Clear-text credential in HTTP header",
                                        severity="high",
                                        details=details,
                            ...
                            for password_param in self.password_params:
                                if password_param in form_values:
                                    ...
                                    details = {
                                        ...
                                        "username_field": username_param_found,
                                        "password_field": password_param,
                                        "username_value": username_value,
                                        "password_value": self._mask_credential(password_value),
                                        "host": host,
                                        "uri": uri,
                                        "method": method,
                                        "detection_location": "form_data"
                                    }
                                    
                                    remediation = "Implement HTTPS if not already in use. Consider using secure authentication methods that don't transmit credentials in plaintext. Review the application's session management and implement secure login mechanisms."
                                    
                                    self.add_red_finding(
                                        src_ip=src_ip,
                                        dst_ip=dst_ip,
                                        description="Clear-text login credentials in form submission",
                                        severity="critical",
                                        details=details,
                                        connection_key=connection_key,
                                        remediation=remediation
    ...
    def _mask_credential(self, value):
        """Mask credential for security in reports"""
